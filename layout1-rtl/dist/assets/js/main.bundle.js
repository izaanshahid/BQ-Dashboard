...
var AatroxCustomizer = function () {
  ...
  var AatroxCustomSettings = document.querySelector('.sidebar-customizer-settings');
  ...
  var initAatroxCustomizer = function () {
    ...
    if (AatroxCustomSettings) {
      AatroxCustomSettings.addEventListener('click', function () {
        if (AatroxCustomizer) {
          ULUtil.hasClass(AatroxCustomizer, 'show') ? ULUtil.removeClass(AatroxCustomizer, 'show') : ULUtil.addClass(AatroxCustomizer, 'show');
        }
      });
    }
...
var Aatrox = function () {
  var initAatrox = function () {
    ...
    ;
    ['.perfect-scrollbar', '.sidebar-left-secondary', '.sidebar-left', '[data-perfect-scrollbar]', '.notification-dropdown', '.dropdown-menu'].forEach(item => {
      console.log(item);
...
var Layout1 = function () {
  var initLayout1 = function () {
    ...
    var $sidebarCompactToggle = document.querySelector('.sidebar-compact-switch');
    ...
    $sidebarFullToggle.addEventListener('click', function (e) {
      ...
      if (ULUtil.hasClass($appAdminWrap, 'sidebar-mobile')) {
        ULUtil.removeClass($appAdminWrap, 'sidebar-mobile'); // e.preventDefault();
...
let layoutTwo = document.querySelector('.app-admin-wrap-layout-2'); // let layoutOne = document.querySelector('.app-admin-wrap-layout-1');

if (layoutTwo) {
  menu.addEventListener('click', () => {
    //secondarySidebar close after hover
    if (ULUtil.hasClass(sidebarLeft, 'open') && ULUtil.hasClass(sidebarLeftSecondary, 'open')) {
      ...
      if (ULUtil.hasClass(sidebarLeft, 'open')) {
        ...
        ULUtil.addClass(mainContentWrap, 'full');
          ...
          ULUtil.removeClass(mainContentWrap, 'full');
          navItem.forEach(e => {
            let dataItem = e.dataset.item; // if the sidebar got active class then it iwill open initial stage
  ...
  navItem.forEach(event => {
    event.addEventListener('mouseover', ex => {
      // e.preventDefault();
      ULUtil.addClass(sidebarLeftSecondary, 'open'); // console.log(elem.target.parentNode);

      let item = event.dataset.item;
      let activeElement = event; //  need to refactor

      navItem.forEach(e => {
        e.classList.remove('active');
        let dataItem = e.dataset.item; // console.log(dataItem);
...
if (layoutThree) {
  menuToggle.addEventListener('click', () => {
    ULUtil.hasClass(layoutThree, 'open') ? ULUtil.removeClass(layoutThree, 'open') : ULUtil.addClass(layoutThree, 'open');
  });
