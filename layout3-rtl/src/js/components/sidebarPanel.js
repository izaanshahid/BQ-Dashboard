  ...
  var initSidebarPanel = function () {
    ...
    toggleButtons.forEach((element) => {
      element.addEventListener('click', function (e) {
        var sidebarPanel = document.getElementById(
          ULUtil.attr(element, 'data-sidebar-panel')
        );

        if (sidebarPanel) {
          ULUtil.hasClass(sidebarPanel, 'open')
            ? ULUtil.removeClass(sidebarPanel, 'open')
            : ULUtil.addClass(sidebarPanel, 'open');
        }
